...
### 拦截规则
- **匹配模式**：`域名`（默认**只拦当前域名**，可勾选扩展子域 / TLD 变体 / 已知镜像站，如 youtube.com ↔ youtu.be）、`包含关键字`、`精确域名`、`通配 *`、`全 URL 精确`。
  - 新规则默认仅拦截添加的域名本身，衍生扩展默认不勾选；可在添加时勾选，或在「设置 → 衍生网站默认策略」调整全局默认。
...
- **主题**：浅色 / 深色 / **跟随系统**（auto，随系统自动切换）；**多语言**：简体中文 / English / 跟随浏览器（设置 → 外观，切换即时生效）。
- 图标角标倒计时（计时拦截/番茄钟剩余分钟）。
...
## 文档
- [使用说明（USAGE.md）](./USAGE.md) —— 安装、功能、快捷键、FAQ
- [隐私政策（PRIVACY.md）](./PRIVACY.md) —— 数据收集与隐私说明

## 赞助 ☕
如果你觉得这个扩展有帮助，可以自愿打赏支持一下（与任何功能解锁无关）：

| 支付宝 | 微信 |
| --- | --- |
| ![支付宝](sponsor/alipay.png) | ![微信](sponsor/wechat.png) |

> 打赏对象：请将你的支付宝/微信收款码截图放到 `sponsor/` 目录（命名 `alipay.png` / `wechat.png`）。
> 合规提示：收款码仅用于自愿打赏，请勿放进扩展内部或商店描述。
